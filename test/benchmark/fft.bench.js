import {signal, fft, analyser, loop, windows} from '../../src/fou.js';
...
let Fs = 44100,
	...
	start = loop({
		...
		process: (s)=> {
			...
			console.log('FFT: Average time per window:', average(time), 'ms');
		}
	});

let x2 = Date.now();
start();
console.log('FFT: Done in', Date.now() - x2, 'ms');
